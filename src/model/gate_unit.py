...
class TabularModel(nn.Module):
    def __init__(self, input_dims, embedding_dim):
        ...
        # Embedding layers
        self.embeddings = nn.ModuleList([nn.Embedding(dim, embedding_dim) for dim in input_dims])
        # Main layers
        self.dropouts = nn.ModuleList([nn.Dropout(0.2) for _ in range(5)])
        self.gates = nn.ModuleList(
            [nn.Linear(embedding_dim * len(input_dims), embedding_dim * len(input_dims)) for _ in range(5)])
        self.linear_layers = nn.ModuleList(
            [nn.Linear(embedding_dim * len(input_dims) * 2, embedding_dim * len(input_dims)) for _ in range(5)])
        # Output layers
        self.output_layers = nn.ModuleList([nn.Linear(embedding_dim * len(input_dims), 1) for _ in range(10)])

    def forward(self, x):
        embeddings = [self.embeddings[i](x[:, i]) for i in range(len(self.input_dims))]
...
#     def forward(self, x):
#         embeddings = [self.embeddings[i](x[:, i]) for i in range(len(self.input_dims[:-1]))]
